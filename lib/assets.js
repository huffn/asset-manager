...
iproto.writeContents = function writeContents(basePath, cb) {
  var finalPath = path.join(basePath, this.getRelativePath());
  // Images should not be gzipped, except 'svg', so we have an exception for them here
  // regex pattern to extract the file extension
  var patt = /\.([0-9a-z]+)(?:[\?#]|$)/i;
  // get extension of current file
  var ext = (finalPath).match(patt);
  // is current file an svg?
  var isSvg = ext.indexOf('svg') !== -1;
  // whether to gzip file or not
  var gzipToggle = (isSvg) ? doGzip : false;
  // encode svg differently than other image content
  var content = (isSvg) ? this.cr.getContentRaw('utf8') : this.cr.getContent();
  utils.writeToFile(finalPath, this.cr.getContent(), gzipToggle, cb);
};
